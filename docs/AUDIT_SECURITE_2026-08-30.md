...
**Modèle de menace retenu :** dépôt **privé** (les PR de fork anonymes ne déclenchent pas les workflows) — le vecteur réaliste n'est donc pas la PR d'un inconnu mais **une dépendance compromise, un compte collaborateur compromis, ou un serveur/MITM sur une source**. Contrôles exécutés localement, aucune correction appliquée. **Cette prémisse a été réexaminée le 03/09 pour le passage en public : elle n'est pas porteuse — voir le statut ci-dessous.**
...
**Statut au 03/09/2026 — S-01, S-03, S-04, S-06 et S-07 fermés. Restent ouverts S-05 et S-08.**

**Le « dépôt privé » du modèle de menace n'est pas porteur, et le passage en public ne change aucune conclusion de cet audit.** La prémisse a été réexaminée plutôt que reconduite. `pipeline.yml` — le seul workflow à cumuler `contents: write`, les secrets et le push vers `master` — ne se déclenche que sur `schedule` et `workflow_dispatch` : **il est inatteignable depuis une PR**, de fork comprise. `validation.yml`, qui tournera sur les PR de fork une fois le dépôt public, est en `contents: read`, ne reçoit aucun secret, et restaure son cache par `actions/cache/restore` — **en lecture seule, il ne peut donc pas écrire de cache** et ne peut pas empoisonner celui que le cron relit ensuite (le cloisonnement des caches par branche l'interdirait de toute façon). Ce que le passage en public change réellement : les octets de ce dépôt deviennent lisibles, scénarios ci-dessous compris. C'est assumé, et c'est la raison pour laquelle les constats qu'un tiers pouvait actionner ont été fermés d'abord.

**S-01 fermé — par arbitrage et vérification, non par correction.** La part corrigible l'a été dans #51. Les deux résidus du 30/08 sont levés : (1) la **séparation en jobs est arbitrée non**, son coût récurrent étant disproportionné à un pipeline de 2 à 4 minutes ; (2) les **droits effectifs des clés S3 sont relevés sur pièce** (02/09) : la clé d'archive porte `PutObject` seul sur le préfixe `archive/`, n'est pas administratrice, et son cloisonnement est établi par un **refus 403 constaté** sur le bucket de la vitrine — le « cloisonnement » que CLAUDE.md affirmait cesse d'être une déclaration pour devenir une propriété observée du compte, ce que S-03 demandait. **Le risque résiduel est assumé, pas éliminé** : une dépendance compromise s'exécuterait toujours avec les secrets de son étape et le droit de pousser `master`. C'est la contrepartie explicite du choix « pip dernières versions », lui-même raisonné (cf. le constat).

**S-03 fermé — le constat est factuellement caduc.** Il supposait `ARCHIVE_ACCESS_KEY/SECRET` « vraisemblablement absents », donc un repli de `depot.configurer` sur `SCW_*` et **une seule clé** servant la vitrine et l'archive. Les deux secrets existent depuis le 30/08/2026 : le repli ne se déclenche jamais. Et la clé d'archive est celle que la docstring appelait de ses vœux — `PutObject` seul sur le préfixe `archive/`, non administratrice, isolée de la vitrine par un **403 constaté**. Le cloisonnement a cessé d'être une condition dans le code pour devenir une propriété observée du compte.

**S-04 fermé.** `defusedxml` remplace `xml.etree` dans les trois modules qui parsent un octet venu du réseau (`fetch`, `provenance`, `prepare`) : le refus est une `EntitiesForbidden` levée **avant** tout développement, et `_valider` la traduit en message lisible. Le risque du remplacement n'était pas le refus mais l'**empreinte** — un décalage d'un seul octet dans la sérialisation canonique aurait invalidé les 20 sources ENTSO-E d'un coup, et fait échouer la collecte sur chacune. Mesuré avant d'écrire la première ligne : empreintes identiques et conformes au manifeste, pour ~7 % de surcoût sur un fichier de 5,2 Mo. Verrous dans `tests/test_octets_hostiles.py`, où chaque test vérifie d'abord que sa charge mord.

**S-06 fermé.** Object Lock activé sur le bucket d'archive : versioning `Enabled`, Object Lock `Enabled`, rétention par défaut **`COMPLIANCE` / 365 jours**. Le constat proposait le mode *gouvernance* ; c'est **conformité** qui a été retenu, plus strict — la gouvernance se contourne avec la permission idoine, elle ne protège donc pas contre l'identité compromise qui est précisément le scénario du constat. Les **242 objets déjà déposés**, qu'aucune règle par défaut ne couvre, ont reçu une rétention explicite (échéance 2027-09-04) relue **objet par objet : 242/242, valeur unique, aucune exception**. L'héritage sur les dépôts neufs est vérifié sur pièce et non déduit — un run du 03/09 à 07:12 UTC a déposé 14 objets, dont la rétention affiche « Conformité jusqu'au 3 septembre 2027 » ; le PUT n'a pas été refusé alors que la clé d'archive ne porte que `PutObject`, ce qui était l'inconnue. Le bucket devient le filet de **prévention** que le constat appelait ; le SHA dans Git reste le filet de **détection**. Contrepartie assumée : plus rien ne s'efface pendant un an, y compris par le propriétaire, et le versioning facture chaque version.

**S-07 fermé sur le risque couvert, avec un résiduel nommé.** Plafond de 512 Mio d'octets écrits dans `_download`, valeur mesurée et non choisie : la plus grosse source réelle pèse 86 Mo, soit six fois la marge, et un verrou interdit de le resserrer sous 256 Mio — trop bas, il ne casserait la collecte que le jour où une source annuelle grossit. Le compte porte sur la sortie d'`iter_bytes()`, donc APRÈS décompression du `Content-Encoding` : **la bombe gzip de transport est arrêtée là aussi**. **Le résiduel, que le verrou d'origine promettait à tort de couvrir** : un `.csv.gz` qui EST la donnée n'explose qu'à la lecture DuckDB dans `prepare` — son volume compressé est plafonné ici, son expansion ne l'est pas. Le rayon reste borné par le timeout de 25 min du job. Chantier distinct, pas un oubli.

**Restent ouverts, et publiés comme tels.** **S-05** (traversée de chemin par le `filename` d'une source) suppose le droit d'écrire dans `sources.yaml`, donc un accès en écriture au dépôt : l'ouvrir au public n'élargit pas ce vecteur. **S-08** (secrets statiques, ni OIDC ni protection d'environnement) est une décision d'architecture, à peser pour elle-même.
